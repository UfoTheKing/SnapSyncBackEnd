...
class FriendshipStatusService {
  public async getFriendshipStatus(loggedUserId: number, friendId: number): Promise<FriendshipStatus> {
    ...
    let isBlocking = await BlockedUsers.query()
      .whereNotDeleted()
      .where({
        userId: loggedUserId,
    ...
    let friendshipHash = await new FriendService().generateFriendshipHash(loggedUserId, friendId);
    let isFriend = await Friends.query()
      .whereNotDeleted()
      .where({
    ...
    if (!isFriend) {
      ...
      let iRequest = await Friends.query()
        .whereNotDeleted()
        .where({
          friendshipHash: friendshipHash,
      ...
      if (!incomingRequest) {
        let oRequest = await Friends.query()
          .whereNotDeleted()
          .where({
            friendshipHash: friendshipHash,
            friendshipStatusId: pendingFriendshipStatus.id,
